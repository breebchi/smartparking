 ...
 */
	function event($object, $event, $observer = null, $options = array()) {
		$isObject = (
			...
			strpos($object, $this->javascriptObject . '(') !== false || strpos($object, '"') !== false ||
			strpos($object, '\'') !== false
		);

        if ($isObject) {
            $b = $this->javascriptObject . "({$object}).bind('{$event}', function(){ {$observer} })";
        } elseif ($object[0] == '\'') {
            //TODO po co to?
            $b = $this->javascriptObject . "(". substr($object, 1) .").bind('{$event}', function(){ {$observer} })";
