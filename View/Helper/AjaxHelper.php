	...
	function remoteFunction($options) {
		if (isset($options['update'])) {
			...
			if (!is_array($options['update'])) {
			    $func = $this->javascriptObject . ".ajax("; 
			    if (!isset($options['complete'])){
			        $options['complete'] = '';
		        }
		        $options['complete'] = $this->javascriptObject . "('#" . $options['update'] . "').$position(request.responseText); " . $options['complete'];
			} else {
				$func = $this->javascriptObject . ".ajax(";
				if (!isset($options['complete'])){
			        $options['complete'] = '';
		        ...
		        $options['complete'] = $this->javascriptObject . "('" . $selectors . "').$position(request.responseText); " . $options['complete'];
			    }
			if (is_array($options['update'])) {
 ...
 */
	function form($params = null, $type = 'post', $options = array()) {
		...
		$defaults = array('model' => $model, 'with' => $this->javascriptObject . "('#{$htmlOptions['id']}').serialize()", 'type' => $type);
 ...
 */
	function submit($title = 'Submit', $options = array()) {
		...
		if (!isset($options['with'])) {
			$options['with'] = $this->javascriptObject . "(this).parents('form:first').serialize()";
		}
 ...
 */
	function observeField($field, $options = array()) {
		if (!isset($options['with'])) {
		    $options['with'] = $this->javascriptObject . "('#$field').serialize()";
		}
		$callback = $this->remoteFunction($options);
 ...
 */
	function observeForm($form, $options = array()) {
		if (!isset($options['with'])) {
			$options['with'] = $this->javascriptObject . "('#$form').serialize()";
		}
		$callback = $this->remoteFunction($options);
 ...
 */
	function autoComplete($field, $url = "", $options = array()) {
		...
		$script = "{$var}" . $this->javascriptObject . "('#{$htmlOptions['id']}').autocomplete('";
 ...
 */
	function drag($id, $options = array()) {
		...
		return $this->Javascript->codeBlock("{$var}" . $this->javascriptObject . "('#$id').draggable( " .$options . ");");
	}
/**
 ...
 */
	function drop($id, $options = array()) {
		...
		return $this->Javascript->codeBlock("{$var}" . $this->javascriptObject . "('#$id').droppable( " .$options . ");");
	}
/**
 ...
 */
	function slider($id, $options = array()) {
		...
		$script = "{$var} " . $this->javascriptObject . "('#$id').slider($options);";
 ...
 */
	function resizable($id, $options = array()) {
		...
		$script = "{$var} " . $this->javascriptObject . "('#$id').resizable($options);";
		return $this->Javascript->codeBlock($script);
	}
/**
 ...
 */
	function selectable($id, $options = array()) {
		...
		$script = "{$var} " . $this->javascriptObject . "('#$id').selectable($options);";
		return $this->Javascript->codeBlock($script);
	}
/**
 ...
 */
	function accordion($id, $options = array()) {
		...
		$script = "{$var} " . $this->javascriptObject . "('#$id').accordion($options);";
		return $this->Javascript->codeBlock($script);
	}
/**
 ...
 */
	function button($id, $options = array()) {
		...
		$script = "{$var} " . $this->javascriptObject . "('#$id').button($options);";
		return $this->Javascript->codeBlock($script);
	}
/**
 ...
 */
	function buttonset($id, $options = array()) {
		...
		$options = $this->_buildOptions($options, $this->buttonOptions);
		$script = "{$var} " . $this->javascriptObject . "('#$id').buttonset($options);";
		return $this->Javascript->codeBlock($script);
	}
/**
 ...
 */
	function datepicker($id, $options = array()) {
		...
		$script = "{$var} " . $this->javascriptObject . "('#$id').datepicker($options);";
		return $this->Javascript->codeBlock($script);
	}
/**
 ...
 */
	function dialog($id, $options = array()) {
		...
		$script = "{$var} " . $this->javascriptObject . "('#$id').dialog($options);";
		return $this->Javascript->codeBlock($script);
	}
/**
 ...
 */
	function progressbar($id, $options = array()) {
		...
		$script = "{$var} " . $this->javascriptObject . "('#$id').progressbar($options);";
		return $this->Javascript->codeBlock($script);
	}
/**
 ...
 */
	function tabs($id, $options = array()) {
		...
		$script = "{$var} " . $this->javascriptObject . "('#$id').tabs($options);";
		return $this->Javascript->codeBlock($script);
	}
/**
 ...
 */
	function editor($id, $url, $options = array()) {
		...
		$script = "{$var}" . $this->javascriptObject . "('#{$id}').editable('{$url}', {$options});";
		return $this->Javascript->codeBlock($script);
	}
/**
	...
	function sortable($id, $options = array()) {
		...
		if (!empty($options['url'])) {
			if (empty($options['with'])) {
			    $options['with'] = $this->javascriptObject . "('#$id').sortable('serialize')";
			}
			$upd = '';
		    if (isset($options['update'])){
		        $upd = $options['update'];
		...
		$result = "{$var} " . $this->javascriptObject . "('#$id').sortable($options);";
	...
	function __optionsForAjax($options) {
	    if (isset($options['indicator'])) {
			if (isset($options['beforeSend'])) {
				...
				$options['beforeSend'] .= $this->javascriptObject . "('#{$options['indicator']}').show()";
			} else {
				$options['beforeSend'] = $this->javascriptObject . "('#{$options['indicator']}').show();";
			}
			if (isset($options['complete'])) {
				...
				$options['complete'] .= $this->javascriptObject . "('#{$options['indicator']}').hide()";
			} else {
				$options['complete'] = $this->javascriptObject . "('#{$options['indicator']}').hide()";
			}
