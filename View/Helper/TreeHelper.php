	...
	function generate ($data, $settings = array ()) {
		...
		foreach ($data as $i => $result) {
			...
			if ($__addType) {
				...
				if ($type) {
					$typeAttributes = $this->__attributes($type, array('data' => $elementData), true, $position);
			...
			if ($itemType) {
				$itemAttributes = $this->__attributes($itemType, $elementData, true, $position);
 ...
 * @return void
 */
	function __attributes($rType, $elementData = array(), $clear = true, $position = 'inner') {
		extract($this->__settings);
		if ($rType == $type) {
		...
		} else if ($autoStyle && $rType != $type) {
			$attributes['class'][] = 'item-'.$this->__settings['data'][$model]['id'].' '.$position;
		}
		if ($attributes) {
